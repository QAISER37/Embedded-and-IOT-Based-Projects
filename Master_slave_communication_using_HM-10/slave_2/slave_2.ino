...
void onReceive(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  ...
  if (cmd == 'B' || cmd == 'C') {
    isActive = true;
    continuous = false;
    Serial.println("ESP3 Activated");
  } else if (cmd == 'A' || cmd == 'D') {
    isActive = false;
    continuous = false;
    Serial.println("ESP3 Stopped");
  } else if (isActive) {
    if (cmd == '1') {
      Serial.println("ESP3: Rotate 90°");
      rotate90();
    } else if (cmd == '2') {
      continuous = true;
      Serial.println("ESP3: Start Continuous Rotation");
    } else if (cmd == '3') {
      continuous = false;
      Serial.println("ESP3: Stop Continuous Rotation");
    }
  }
}
